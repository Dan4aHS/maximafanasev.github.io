...
# ^ начало строки
print(ww.search('Hello motherf...').group())
print(ww.search('motherf... Hello') == None)

ee = re.compile(r'Hello$')
# $ заканчивается этим
print(ee.search('Hello motherf...') == None)
print(ee.search('motherf... HHHHello').group())

rr = re.compile(r'.at')
# at групповой символ
print(rr.search('The cat in the hat').group())
print(rr.findall('The cat in the hat'))

tt = re.compile(r'First Name: (.*) Last Name: (.*)')
# at групповой символ
print(tt.search('First Name: Max Last Name: Afanasev').groups())

tt = re.compile(r'<.*?>')
# at групповой символ
print(tt.search('<aaaa> bbb>').group())
